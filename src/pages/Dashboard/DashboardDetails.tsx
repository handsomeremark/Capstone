...
const DashboardDetails: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchTotalProducts = async () => {
      try {
        const response = await fetch('http://localhost:5001/total-products');
        const data = await response.json();
  ...
  const displayTotalUsers = totalUsers !== null ? totalUsers.toLocaleString() : 'Loading...';
  const displayTotalProducts = totalProducts !== null ? totalProducts.toLocaleString() : 'Loading...';


  return (
    <>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        <CardDataStats title="Total Orders" total="₱3k" rate="0.43%" levelUp>
          <svg
        ...
        <CardDataStats title="Total Profit" total="₱1k" rate="4.35%" levelUp>
          <svg
            className="fill-primary dark:fill-white"
            width="20"
            height="22"
